  ...
  runslash: async (client, interaction, guildData, player, prefix) => {
    try {
      ...
      const gggg = new MessageEmbed()
        .setColor("#303037")
        .setDescription(`${emoji.msg.warn} **Do you really want to reset all Settings**?\n*Reply with:* **__\`yes\`__**`)
      let themsg = interaction.reply({ embeds: [gggg] }).then((msg) => {
        //wait for answer of the right user
        interaction.channel.awaitMessages(m => m.member.id === interaction.member.id, {
          max: 1,
          time: 30 * 1000,
          errors: ['time']
        })
          ...
          .then(async collected => {
            ...
            if (collected.first().content.toLowerCase() === `yes`) {
              ...
              const rrr = new MessageEmbed()
                .setColor("#303037")
                .setDescription(`${emoji.msg.SUCCESS} Resetted everything!`)
              return interaction.reply({ embeds: [rrr] });
            }
            //if an error happens, reply
          }).catch(e => {
            console.log(String(e.stack).yellow)
