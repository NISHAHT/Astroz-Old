...
module.exports = {
  ...
  runslash: async (client, interaction, guildData, player, prefix) => {
    const emem  = new MessageEmbed()
    .setAuthor(`Saved Song:`, interaction.member.displayAvatarURL({
      dynamic: true
    }))
    .setURL(player.queue.current.uri)
    .setColor("#2F3136")
    ...
    .setFooter(`Requested by: ${player.queue.current.requester.tag} | in: ${interaction.guild.name}`, player.queue.current.requester.displayAvatarURL({
      dynamic: true
    }))
    interaction.member.send({embeds: [emem]}).catch(e=>{
      const ememe = new MessageEmbed()
      .setColor(ee.wrongcolor)
      .setDescription(`**${emoji.msg.ERROR} Your Dm's are disabled**`)
      return interaction.reply({embeds: [ememe]})
  })}
}