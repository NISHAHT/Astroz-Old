    ...
    try {
      ...
      if (!player){
        if(interaction.guild.me.voice.channel) {
          ...
          .setDescription(`${emoji.msg.stop} Stopped and left your Channel`)
          .setColor("#2F3136")
          return interaction.reply({embeds: [pppp]});
        }
        else {
          const opoppp = new MessageEmbed()
          .setColor(ee.wrongcolor)
          .setDescription(`${emoji.msg.ERROR} No song is currently playing in this guild.`)
          return interaction.reply({embeds: [opoppp]});
        }
      }
      ...
      if (channel.id !== player.voiceChannel) {
        ...
        return interaction.reply({embeds: [noi]})
      }
      if (!player.queue.current) {
        const no = new MessageEmbed()
        .setColor(ee.wrongcolor)
      ...
      if(player.queue.size == 0) {
        if(player.get("autoplay")) {
          const idkd = new MessageEmbed()
          .setDescription(`${emoji.msg.skip_track} Skipped **${player.queue.current.title}** by \`${interaction.member.tag}\``)
          .setColor(ee.color)
          interaction.reply({embeds: [idkd]});
          return autoplay(client, player, "skip")
        } else {
          player.stop();
          const idkd = new MessageEmbed()
