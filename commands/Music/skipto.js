...
module.exports = {
  ...
  options: [
    {
      name: 'number',
      description: 'Provide Track Number For Skip!',
      required: true,
      type: 'STRING'
    }
  ],
  ...
  runslash: async (client, interaction, guildData, player, prefix) => {
    try {
      ...
      if (Number(choose) > player.queue.size) {
        ...
        .setColor(ee.wrongcolor)
        .setDescription(`${emoji.msg.ERROR} That song is not in the queue, sorry!`)
        return interaction.reply({embeds: [rrt]})
      }
      //remove all tracks to the jumped song
      player.queue.remove(0, Number(choose) - 1);
      //stop the player
      player.stop()
      //Send Success Message
      const popo = new MessageEmbed()
      .setDescription(`${emoji.msg.SUCCESS} Jumped to the: \`${choose}\` Song\n\n${emoji.msg.skip_track} Skipped \`${Number(choose)}\` Songs`)
      .setColor("#2F3136")
      return interaction.reply({embeds: [popo]});
    } catch (e) {
      console.log(String(e.stack).bgRed)
      const emesdf = new MessageEmbed()
      .setColor(ee.wrongcolor)
