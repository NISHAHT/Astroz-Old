...
module.exports = {
  ...
  description: `Leaves A Guild  [Owner Only]`,
  usage: `leaveguild <guild id>`,
  options: [
    {
      name: 'id',
      description: 'Provide Guild Id To Leave [ Owner Only ]!',
      required: true,
      type: 'STRING'
    }
  ],
  ...
  runslash: async (client, interaction, guildData, player, prefix) => {
    ...
    try {
      const choose = interaction.options.getString("id")
      const gleave = choose;
 
      if (!gleave) {
        return interaction.reply({content: "Please provide an id"});
      }
         
      const guild = client.guilds.cache.find((g) => g.id === gleave);
         
      if (!guild) {
        return interaction.reply({content: "That guild wasn't found"});
      }
         
      try {
        await guild.leave();
        interaction.reply({content: `I have left the Guild: **${guild.name}**`});
      } catch (error) {
        console.error(error);
        return interaction.reply({content: "I Couldent leave the Guild!"});
      } 
    } catch (e) {
      console.log(String(e.stack).bgRed)
      const emesdf = new MessageEmbed()
