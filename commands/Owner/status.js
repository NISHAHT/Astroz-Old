...
module.exports = {
  ...
  options: [
    {
      name: 'type',
      description: 'Provide Type For Status [ Owner Only ]!',
      required: true,
      type: 'STRING'
    },
    {
      name: 'text',
      description: 'Provide Text For Status [ Owner Only ]!',
      required: true,
      type: 'STRING'
    }
  ],
  ...
  runslash: async (client, interaction, guildData, player, prefix) => {
    ...
    try {
      ...
      const choose1 = interaction.options.getString("text")

      let status = config
      status.status.text = choose1.join(` `).substr(0, 50);
      status.status.type = choose.toUpperCase();
      client.user.setActivity(choose1.join(` `).substr(0, 50), {
        type: choose.toUpperCase(),
        url: config.status.url
      })
      fs.writeFile(`./botconfig/config.json`, JSON.stringify(status, null, 3), (e) => {
        if (e) {
          ...
          return interaction.reply({embeds: [nope]})
        }
        const opop = new MessageEmbed()
        .setColor(ee.color)
        .setDescription(`${emoji.msg.SUCCESS} Successfully set the new Status`)
